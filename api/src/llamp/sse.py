...
class RobustStructuredChatOutputParser(StructuredChatOutputParser):
    """A more robust output parser that handles GPT-4o outputs without code blocks."""

    def parse(self, text: str) -> AgentAction | AgentFinish:
        # First try the default parser (handles ```json ... ``` blocks)
        try:
            return super().parse(text)
        except Exception:
            pass

        # Try to find raw JSON in the text (no code block wrapping)
        json_match = re.search(r'\{\s*"action"\s*:\s*"[^"]*"[\s\S]*?\}', text)
        if json_match:
            try:
        ...
        # Detect if the text looks like the agent is still thinking (not a final answer)
        intent_phrases = [
            "i will", "i'll", "let me", "i need to", "i should",
            "i can", "i want to", "let's", "we need", "we should",
            "i'm going to", "next step", "first,", "now i",
        ]
        text_lower = text.lower().strip()
        for phrase in intent_phrases:
            if phrase in text_lower:
                raise OutputParserException(
                    f"Could not parse LLM output. The text appears to be a thought, not an action. "
                    f"Please respond with a valid JSON action blob in the format: "
                    f'```json\n{{"action": "tool_name", "action_input": "input"}}\n``` '
                    f"or ```json\n{{\"action\": \"Final Answer\", \"action_input\": \"your answer\"}}\n```"
                )

        # If it doesn't look like a thought, treat it as a final answer
...
async def listen_to_pubsub(pubsub: PubSub, finish_event: asyncio.Event):
    while not finish_event.is_set():
        message = pubsub.get_message()
        if message and message["type"] == "message":
            data = message["data"].decode()
            if data == "AGENT_FINISH":
                finish_event.set()
                break
            yield data
...
async def agent_stream(
    ...
    output_parser = RobustStructuredChatOutputParser()
    ...
    async for message in listen_to_pubsub(pubsub, finish_event):
        yield message.encode("utf-8")

    # Ensure agent task is completed before exiting
    try:
        await agent_task
